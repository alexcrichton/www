module Synthesis
  class AssetPackage
    ...
    @@asset_packages_yml = $asset_packages_yml ||
      (File.exists?("#{SINATRA_ROOT}/compress/asset_packages.yml") ?
        YAML.load_file("#{SINATRA_ROOT}/compress/asset_packages.yml") :  nil)
    ...
    class << self
      ...
      def create_yml
        unless File.exists?("#{SINATRA_ROOT}/compress/asset_packages.yml")
          asset_yml = Hash.new

          asset_yml['javascripts'] = [{"base" => build_file_list("#{SINATRA_ROOT}/public/javascripts", "js")}]
          asset_yml['stylesheets'] = [{"base" => build_file_list("#{SINATRA_ROOT}/public/stylesheets", "css")}]

          File.open("#{SINATRA_ROOT}/compress/asset_packages.yml", "w") do |out|
    ...
    def initialize(asset_type, package_hash)
      ...
      @asset_path = ($asset_base_path ? "#{$asset_base_path}/" : "#{SINATRA_ROOT}/public/") +
    ...
    private
      ...
      def compress_js(source)
        jsmin_path = "#{SINATRA_ROOT}/compress"
        tmp_path = "#{SINATRA_ROOT}/tmp/#{@target}_packaged"
